...
public class GuiMainClient {
	...
	public static void BuildLoginWindow() {
		...
		imagePanel.setBackground(createImageIcon("hypebig.png", "logo")
				.getImage());
				...
				if (!isConnected)
					JOptionPane
							.showMessageDialog(null,
									"You are not connected to a server. Please connect and try again.");
				else
					RegisterRequest();
			}
		});
	...
	// Method that creates and the main window
	public static void BuildMainWindow(String username) {
		...
		usernameBar.setBackground(createImageIcon("topbg.png", "top bar bg")
				.getImage());
		...
		removeFriendItem.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent event) {
				if (deleteTarget != null)
					DeleteFriendRequest(deleteTarget);
				System.out.println("removing friend"); // probably keep a
														// variable that is set
														// to the target friend
														// when the popup menu
														// is spawned
			}
		});
		...
		onlineUsers.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent evt) {
				...
				if (item.mode == FriendListItemMode.REQUEST
						&& evt.getClickCount() == 1) { // click friend request
														// (accept/decline)
				...
				} else if (item.mode == FriendListItemMode.FRIEND
						&& item.status == 1 && evt.getClickCount() == 2) { // double
																			// click
																			// friend
																			// (call)
	...
	public static void BuildAddFriendWindow() {
		...
		addFriendWindow.getContentPane().setPreferredSize(
	...
	//Creates the call inquiry window
	...
	//Creates the call in progress window
	...
	//Disconnects the client from the server, ends a call, exits the system
	...
	//Sends a call request, builds the call in progress window
	//Starts the calling sound
	...
	//Sends end call request; stops the calling sound if on;
	//closes the callinprogwindow
	...
	//Accepts a received call; sets the call in prog window to visible
	...
	//initiates the p2p voice component
	...
	//closes the p2p voice component
